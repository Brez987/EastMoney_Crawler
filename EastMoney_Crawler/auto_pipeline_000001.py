...
三阶段执行：
  ...
  Stage 2: 流式补爬正文（财富号+缺失）→ 爬取评论
  Stage 3: 合并 CSV → 上传百度网盘 → 清理本地数据

用法：
  python3 auto_pipeline_000001.py --stage 1   # tmux 会话 1
  python3 auto_pipeline_000001.py --stage 2   # tmux 会话 2（等 stage1 完成）
  python3 auto_pipeline_000001.py --stage 3   # tmux 会话 3（等 stage2 完成）
"""
import os
import sys
import csv
...
def run_stage1():
    ...
    if not os.path.exists(base_path):
        if os.path.exists(RAR_FILE):
            ...
            local_csv = os.path.join(os.path.dirname(__file__), f'{STOCK_CODE}.csv')
            if os.path.exists(local_csv):
                print(f'\n[Stage 1-1] RAR 不存在，使用本地 CSV: {local_csv}')
                shutil.copy2(local_csv, base_path)
                file_size = os.path.getsize(base_path) / 1024 / 1024
                print(f'  ✓ 已复制到: {base_path} ({file_size:.1f} MB)')
            else:
                print(f'[错误] 数据.rar 和 {STOCK_CODE}.csv 均不存在，无法继续')
                return False
    else:
...
def load_posts_for_detail_crawl(csv_paths: list) -> list:
    """从 CSV 加载需要补爬正文的帖子列表（仅加载必要字段，内存友好）

    补爬逻辑（仅补爬缺失的，不重复爬取已有的）：
    1. 财富号帖子：仅当 content 为空时补爬
    2. 非财富号帖子：仅当 content 为空时补爬
    """
    ...
    print(f'  ✓ 缺失正文: {empty_count}, 总计待爬: {len(posts)}')
    return posts
...
def _append_content_delta(stock_code: str, post_id: str, content: str):
    """追加单条正文更新到 delta 文件。"""
    if not content:
        return
    path = _content_delta_path(stock_code)
...
def crawl_post_detail_csv(stock_code: str, csv_paths: list):
    """从 CSV 读取并补爬帖子正文（支持断点续爬）"""
    print(f'\n[Stage 2-1] 爬取帖子正文（仅爬取 content 为空的帖子）...')
    posts = load_posts_for_detail_crawl(csv_paths)
    ...
    CHECKPOINT_INTERVAL = 50  # 每 50 条保存一次检查点，CSV 最后统一写回
    crawl_count = 0
    batch_done = set()  # 当前批次完成的 post_id
    ...
    def update_callback(post_id, update_data):
        ...
        crawl_count += 1
        batch_done.add(pid)
        # 只保存检查点；CSV 最后统一写回，避免频繁重写大文件。
        if crawl_count % CHECKPOINT_INTERVAL == 0:
    ...
    if batch_done:
        _save_checkpoint(stock_code, batch_done)
        batch_done.clear()
    _delete_checkpoint(stock_code)  # 全部完成，删除检查点
    _delete_content_delta(stock_code)

    print(f'  ✓ 正文爬取完成，共成功爬取 {crawl_count}/{len(posts)} 条')
    return True
...
def crawl_comments_csv(stock_code: str, csv_paths: list):
    """从 CSV 读取帖子信息并爬取评论（支持多个 CSV 文件）"""
    print(f'\n[Stage 2-2] 爬取评论信息 ...')

    # 从所有 CSV 读取需要爬评论的帖子（comment_num > 0）
    comment_posts = []
    seen_ids = set()
    for csv_path in csv_paths:
        if not os.path.exists(csv_path):
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                seen_ids.add(pid)
                try:
                    cnum = int(row.get('coment_count', 0))
                except ValueError:
                    cnum = 0
                if cnum > 0:
    ...
    print(f'  需要爬取 {len(comment_posts)} 个帖子的评论')

    # 使用 CommentCrawler，但直接注入 DataFrame 避免查询 MongoDB
    comment_crawler = CommentCrawler(stock_code)
    comment_crawler.post_df = pd.DataFrame(comment_posts)

    # 评论断点续爬
    comment_cp = os.path.join(TEMP_DIR, f'{stock_code}_comments_checkpoint.json')
    comment_crawler.crawl_comment_info(checkpoint_path=comment_cp)

    # 全部完成后删除评论检查点
    if os.path.exists(comment_cp):
        os.remove(comment_cp)
...
def run_stage2():
    print(f'\n{"="*60}')
    print(f'[Stage 2] {STOCK_CODE} 正文补爬 + 评论爬取')
    print(f'{"="*60}')
    ...
    # 1. 补爬正文（仅爬取 content 为空的帖子）
    crawl_post_detail_csv(STOCK_CODE, csv_paths)

    # 2. 爬取评论（不阻塞 Stage 完成标记）
    try:
        crawl_comments_csv(STOCK_CODE, csv_paths)
    except Exception as e:
        print(f'[警告] 评论爬取出错: {e}')
        print(f'  正文爬取已完成，将继续标记 Stage 2 完成')

    set_flag('stage2')
    print(f'\n{"="*60}')
    print(f'[Stage 2] 完成！请在新 tmux 会话中运行 Stage 3')
    print(f'{"="*60}')
    return True
...
def cleanup_all(stock_code: str):
    ...
    files_to_remove = [
        ...
        comment_csv_path(stock_code),
        os.path.join(TEMP_DIR, f'{stock_code}.csv'),
    ]
...
def run_stage3():
    ...
    # 2. 导出评论
    ccsv = export_comments(STOCK_CODE)

    # 3. 输出到 data/ 目录（本地检查模式）或上传百度网盘
    if SKIP_BAIDU_UPLOAD:
        os.makedirs(DATA_OUTPUT_DIR, exist_ok=True)
        for src, label in [(post_csv, '帖子'), (ccsv, '评论')]:
            if src and os.path.exists(src):
                dst = os.path.join(DATA_OUTPUT_DIR, os.path.basename(src))
                shutil.copy2(src, dst)
                size_mb = os.path.getsize(dst) / 1024 / 1024
                print(f'  ✓ [{label}] 已复制到: {dst} ({size_mb:.1f} MB)')
        print(f'\n[Stage 3] 跳过上传和清理（SKIP_BAIDU_UPLOAD=True），文件已保存至 data/ 目录')
        print(f'  检查完毕后，将 SKIP_BAIDU_UPLOAD 改为 False 即可恢复网盘上传')
    else:
        # 3. 上传百度网盘
        if not upload_to_baidu(STOCK_CODE, post_csv, ccsv):
...
def main():
    ...
    parser.add_argument('--stage', type=int, choices=[1, 2, 3], required=True,
                        help='运行阶段: 1=提取+列表爬取, 2=正文+评论爬取, 3=整合上传清理')
